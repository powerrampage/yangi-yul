...
import { LabelAndError, UploadFile } from "@/components";
...
const FormSection = () => {
  ...
  const { handleSubmit, control, watch, formState } = useForm<any>({ defaultValues: {} });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className={classes.box}>
        <Row gutter={[30, 30]}>
          <Col xs={24} sm={12}>
            <LabelAndError label={t("Pasport rasmi (pdf, png, jpg, jpeg formatda, maksimal 5 Mb)")} error={errors?.passportFile} required>
              <UploadFile
                field="passportFile"
                control={control}
                file={watch("passportFile")}
          ...
          <Col xs={24} sm={12}>
            <LabelAndError label={t("Pasport bilan selfi surat (pdf, png, jpg, jpeg formatda, maksimal 5 Mb)")} error={errors?.selphieFile} required>
              <UploadFile
                field="selphieFile"
                control={control}
                file={watch("selphieFile")}
                inputProps={{ accept: ".pdf, .png, .jpg, .jpeg" }}
                isLarge
                required
              />
